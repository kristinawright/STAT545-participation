...
replace pipe %>% with plus +, replace geom_point to geom_jitter, add alpha for transparency to fix over plotting (overlap), and add line of best fit (linear model):  (rounded version anyways for geom_point so still accurate)
...
scales free means scaled each graph for each graph ->  "free_y" frees only y axis
...
force all into 1 row
larger the pop the larger the bubbles
