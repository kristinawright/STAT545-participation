...
5. Rename `continent` to `cont`. 2 ways to rename:
...
Now using with pipes: (easier to read code with pipes)
...
2. Your turn: of those rows filtered from step 1., only take data from Asia.  3 ways below:
no select because only takes certain rows
...
Try the same thing, but with `transmute` (drops all other variables). And round to 2 digits after decimal.
...
Example: Suppose Canada's 1952 life expectancy was mistakenly entered as 68.8 in the data frame, but is actually 70. Fix it using `if_else` and `mutate`. --> lifeExp to keep other values for else 
